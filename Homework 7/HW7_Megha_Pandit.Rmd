...
```{r}
data <- hiv.data[,list(alph = coef(fit1)[1], bet = coef(fit1)[2]), by = newpid]
...
```{r}
fit4 <- lmer(CD4PCT ~ time + (1 + time|newpid), data = hiv.data)
display(fit4)
```
**_The average of time trends is -2.82 (estimated coefficient for time) with a standard error of 0.61. Thus, it can be estimated that most of the children may have declining levels of CD4 levels during this time period._**
...
```{r}
fit5 <- lmer(CD4PCT ~ time + (1|treatment) + (1|age.baseline), data = hiv.data)
display(fit5)
```
...
colnames(olymp_final) <- paste(c("Technical Merit Score", "Artistic Impression Score", "Skater ID", "Judge ID"))
olymp_final <- as.matrix(olymp_final)
