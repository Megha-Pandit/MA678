...
#Model 1
fit_v1 <- glm(vote_rep ~ age + female + ideo7 + religion + income + occup1 + educ1, data = nes5200_dt_s, family = binomial(link = "logit"))
res_v1 <- resid(fit_v1)
...
#Model 2
fit_v2 <- glm(vote_rep ~ age + female + income + religion + state + occup1, data = nes5200_dt_s, family = binomial(link = "logit"))
res_v2 <- resid(fit_v2)
...
fit_v3 <- glm(vote_rep ~ female + ideo7 + religion + income + occup1 + educ1 + female:educ1, data = nes5200_dt_s, family = binomial(link = "logit"))
res_v3 <- resid(fit_v3)
...
fit_v4 <- glm(vote_rep ~ age + female + income + religion + state + occup1 + state:educ1, data = nes5200_dt_s, family = binomial(link = "logit"))
res_v4 <- resid(fit_v4)
...
summary(fit_v3)
```
**_The model has a residual deviance of 1115.8 and an AIC of 1167.8. The coefficients of sex (female/male), religion3, religion4, income4, income5, occup15, educ12, educ13, educ14, and the interaction terms are statistically significant. An increase in the income leads to an increase in the probability of voting for a Republican. This makes sense since people who are on the richer side prefer voting for Republicans. Republicans are also supported by farm laborers since a positive change in occup15 leads to a positive change in the probability. The interaction term is statistically significant._**
...
```{r, include=FALSE}
dist100 <- wells_dt$dist[wells_dt$dist < 100]

dist100_200 <- wells_dt$dist[wells_dt$dist >= 100 & wells_dt$dist < 200]

dist200 <- wells_dt$dist[wells_dt$dist > 200]

#Since the lengths of these dist variables differ from each other,
#For fitting a logistic regression with these distances
#we need to convert them into binary variables
```
...
**_The coefficient estimates for asian and hisp are each not 2 standard deviations away from zero. They are not statistically sifgnificant. But adding asian and hisp reduces the residual deviance by more than the expected value. However, the variables asian, black and hisp can be represented by just the variable race. We can see that the constant term and the coefficients for race and black are statistically significant, with relatively small standard errors._**
...
```{r}
fit_r1 <- glm(y ~ race + black + hisp + floor + defects + bldg + poor, data = apt_dt, family = binomial(link = "logit"))
summary(fit_r1)
```
...
```
**_The deviance does not change with the addition of pure noise to the model._**
...
**_The intercept is_** $logit(0.27) = -0.995$ **_and the logistic regression model is given by_** $P(y=1) = logit^{-1}(-0.995 + 0.497x)$ **_where income is measured in units of $10,000 and y is the probability that a child graduates from high school._**
...
ggplot(data=data.frame(z=z), aes(x=z))+
  geom_density() +
  geom_ribbon(data=subset(data.frame(z=z), z>0), aes(ymax=invlogit(z)), ymin=0, fill="blue", alpha = 0.3)
```
 ...
 **_The models show the estimates for coefficients of logistic regression representing the probability of a Republican vote for the presidential elections. In 1964, the coefficient of black became statistically insignificant because of complete separation. All the African-Americans supported Democrats and hence, the difference._**
